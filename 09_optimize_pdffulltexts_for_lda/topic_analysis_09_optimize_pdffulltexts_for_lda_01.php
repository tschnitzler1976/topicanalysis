	...
	if(isset($_POST["select_existing_topic_analysis"])){
		...
		//We have more than 0 searchstrings for this topic analysis. Thus continue.
			...
			/*We have to look in each row of table search_results whether the value of the column
			 ...
			 Thus we prevent the user from proceeding. The same for the status of preprocessed or not preprocessed pdffulltexts.
			 If the pdffullltexts were not preprocessed then we will not optimize because there are no values in column
			 "pdffulltext_for_lda" to optimize.*/
			...
			for($zaehler2=0;$zaehler2<$zaehler;$zaehler2++){
				...
				$returndbselect2=dbselect($returndbconnect,"search_results", "id_search_strings='$searchstringsids[$zaehler2]' AND pdffulltext_as_text_extracted<>'' AND preprocessing_pdffulltext_already_done=0 AND exclude=0","id");			
			...
			if($exclusionhappenedbefore==true){			
				if($preprocessinghappenedbefore==true){
					for($zaehler2=0;$zaehler2<$zaehler;$zaehler2++){
						$returndbselect=dbselect($returndbconnect,"search_results", "id_search_strings='$searchstringsids[$zaehler2]' AND pdffulltext_for_lda <>'' AND preprocessing_pdffulltext_already_done=1 AND exclude=0","conference");			
						$dbnumrowsvar2=dbnumrows($returndbselect);
						if($dbnumrowsvar2>0){
							while(list($id,$id_search_strings,$exclude,$authors_row,$title_row,$conference_row,$year_row,$first_link_to_abstracttext,$abstracttext,$abstracttext_for_lda,$first_link_to_pdffulltext,$path_to_pdffulltext,$pdffulltext_as_text,$pdffulltext_as_text_extracted,$pdffulltext_for_lda_row) = mysqli_fetch_row($returndbselect)){							
								/*We fetch the id in order to address the pdffulltext_for_lda that manually can be updated by the user.*/
								$id_search_result=$id;
								$authors=$authors_row;
								$title=$title_row;
								$conference=$conference_row;
								$year=$year_row;
								$pdffulltextforlda=base64_decode($pdffulltext_for_lda_row);
								...
								<textarea name="textarea_pdffulltext_as_text_extracted_id_' . $id_search_result . '" style="width: 657px; height: 231px">' . $pdffulltextforlda .'</textarea></td>
					...
					if($noabstracts==false){
						$headline='<tr>
							<td style="width: 528px; height: 23px;" class="auto-style1">Optimization form for pdffulltexts for lda for the topic analysis "' . $topicanalysisname . '".</td>
							<td style="height: 23px">&nbsp;</td>
						</tr><tr>
							<td style="width: 528px; height: 23px;" class="auto-style1">If you find text parts that are not desireable please filter them out.</td>
							<td style="height: 23px">&nbsp;</td>
						</tr><tr>
			   ...
			   {font-weight:normal;color:#181818;background-color:#fffccf}b.b4{font-weight:normal;color:#0c0c0c;background-color:#fffccf}b.b2{font-weight:normal;color:#242424;background-color:#fffeef}
				...
				<body>
					<form method="post" action="topic_analysis_09_optimize_pdffulltexts_for_lda_02.php">
					<table style="width: 100%">
						...
						echo 'No pdffulltexts for lda found for this topic analysis. Please go back to the <a href="../topic_analysis_00_menu.php">menu</a>';			
					}
				}else{
					echo 'Please <a href="../topic_analysis_07_b_preprocessing_pdffulltexts.php">preprocess</a> first.';
				}
			}else{
				echo 'Please run the <a href="../topic_analysis_06_exclude_search_results.php">exclusion procedure</a> before optimizing pdffulltext for lda. Thank you!';
			}
		}else{
