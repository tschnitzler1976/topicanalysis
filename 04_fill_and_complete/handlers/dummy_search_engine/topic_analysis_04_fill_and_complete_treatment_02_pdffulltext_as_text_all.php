<?php
	...
	if(isset($_GET["topicanalysisid"])){	
		...
		/*We complete any field of the column "pdffulltext_as_text" in table "search_strings" if the related field
		...
		//We have more than 0 searchstrings for this topic analysis. Thus continue.
			...
			//SELECT the filled path_to_pdffulltexts for each searchstringid of this topicanalysisid.	
			for($zaehler2=0;$zaehler2<$zaehler;$zaehler2++){
				...
				if($dbnumrowsvar2>0){
					while(list($id,$id_search_strings,$exclude,$authors,$title,$conference,$year,$first_link_to_abstracttext,$abstracttext,$abstracttext_for_lda,$first_link_to_pdffulltext,$path_to_pdffulltext_row) = mysqli_fetch_row($returndbselect)){							
			...
			$headline='<tr>
				<td style="width: 528px; height: 23px;" class="auto-style1">Completion of the texts for the pdffulltexts for
				 the topic analysis "' . $topicanalysisname . '" if zero pdffulltexts_as_text have already been provided related to
				 the provided path_to_pdffulltexts.</td>
